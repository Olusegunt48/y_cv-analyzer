...
def determine_base_score(job_description):
    ...
    elif any(level in job_description.lower() for level in ["junior", "entry"]):
        return 40  # Lower base for junior or entry-level roles
    else:
        return 50  # Neutral base if no seniority is mentioned
...
def education_score(job_description, education):
    """Assigns a score based on how well the education level and related fields match the job requirements."""
    education_levels = ["phd", "master", "bachelor", "associate", "diploma", "degree"]
    related_fields_keywords = extract_keywords(job_description)
    ...
    # Map of related disciplines for flexibility
    related_disciplines = {
        "data science": ["computer science", "statistics", "mathematics", "informatics", "engineering", "economics"],
        "business analytics": ["economics", "statistics", "business administration", "data science"]
    }

    matched_field = False
    for keyword in related_fields_keywords:
        for field, related_list in related_disciplines.items():
            if keyword in field or any(rel in education.lower() for rel in related_list):
                matched_field = True
                break
    ...
    if candidate_index <= job_index:
        return 15 if matched_field else 10
    elif candidate_index - job_index == 1:
        return 10 if matched_field else 5
    else:
...
def analyze_resume(resume_data, job_description):
    ...
    # Always perform analysis regardless of seniority level
